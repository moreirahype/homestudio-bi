...
function doPost(e) {
  ...
  if (!validation.ok) return outputJson_(validation);
  const row = normalizeWebhook_(payload);
  if (!row) return outputJson_({ ok: true, ignored: true });
...
function normalizeWebhook_(payload) {
  ...
  return [
    isCakto ? pickValue_(payload, ['data.id', 'id']) || Utilities.getUuid() : Utilities.getUuid(),
...
function validateWebhook_(payload) {
  if (!isCaktoPayload_(payload)) return { ok: true };
  const expectedSecret = PropertiesService.getScriptProperties().getProperty('CAKTO_WEBHOOK_SECRET');
  if (!expectedSecret) return { ok: true };
  const receivedSecret = String(pickValue_(payload, ['hsbi_key', 'cakto_key']) || '');
